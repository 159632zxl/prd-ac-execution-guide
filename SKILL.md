...
Use canonical identifiers with no leading or trailing whitespace. All downstream `requirement_refs` and Test Chain `ac_refs` must be declared by `source_coverage`. A node, edge, or contract whose status is `observed`, `changed`, `implemented`, or `verified` requires non-empty `verification_evidence`. Every source-coverage edge reference must include both endpoints in `target_node_refs`.
